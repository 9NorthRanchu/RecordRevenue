...
**ทดสอบแล้ว 71 เคสด้วย Chromium จริง** — `node frontend/trip-unified-prototype/live-mode.test.mjs` · เสิร์ฟไฟล์เอง + API ปลอมที่คืนรูปเดียวกับของจริง **ไม่ยิงฐานจริงเลย** · ครอบคลุมทั้งโหมดปกติ · โหมดจริง · กรณี API ล้ม · และ console สะอาด
...
🔴 **ความเสี่ยงที่ยังไม่ได้แก้ — รหัสผ่านเก็บเป็นข้อความธรรมดา**

`/api/login` เทียบด้วย `WHERE ... AND password = ?` ตรง ๆ ไม่มี hash · ใครอ่านตาราง `Users` ได้ก็เห็นรหัสทันที นี่คือเหตุผลที่ dump หลุดเมื่อ 2026-07-27 ถึงร้ายแรง · ควรย้ายไปเก็บเป็น hash (bcrypt/PBKDF2) แต่ต้อง migrate ของเดิมพร้อมกัน จึงยังไม่ได้ทำ
