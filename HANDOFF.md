...
🔴 **แต่นี่ยังไม่ใช่การยืนยันตัวตนจริง** — API เชื่อ header `x-user-id` ตรง ๆ ไม่มีการตรวจสอบใด ๆ ใครรู้ `user_id` ของคนในบ้านก็เรียก API อ่านข้อมูลการเงินทั้งครอบครัวได้จาก `curl` โดยไม่ต้องมีรหัสผ่าน · การอ่านจาก session แก้เรื่อง UX กับการไม่โชว์ id ในลิงก์เท่านั้น **ไม่ได้ปิดช่องนี้** · ทางแก้จริงคือออก token ตอนล็อกอินแล้วให้ทุก endpoint ตรวจ token แทนการเชื่อ header — **ยังไม่ได้ทำ**

**ทดสอบแล้ว 83 เคสด้วย Chromium จริง** — `node frontend/trip-unified-prototype/live-mode.test.mjs` · เสิร์ฟไฟล์เอง + API ปลอมที่คืนรูปเดียวกับของจริง **ไม่ยิงฐานจริงเลย** · ครอบคลุมทั้งโหมดปกติ · โหมดจริง · กรณี API ล้ม · และ console สะอาด
