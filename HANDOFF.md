...
**ทดสอบแล้ว 44 เคส** — `node backend/test/unified-trip-write.test.mjs` · ใช้ sqlite ในหน่วยความจำ (`node:sqlite`) + shim ที่เลียน API ของ D1 · **ไม่แตะฐานจริงทั้ง local และ remote** · โหลด `src/unified-trip.js` ผ่าน data: URL จึงทดสอบไฟล์จริงเสมอ ไม่ใช่สำเนา · มีเคสสร้าง 20 ใบรวดเดียวเพื่อกันบั๊ก id ชนกลับมา
