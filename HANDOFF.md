...
**ทดสอบแล้ว 102 เคสด้วย Chromium จริง**

### รันเทสทั้งหมด

```
npm test            # ทุกชุด (270 เคส)
npm run test:api    # 217 — ไม่ต้องลงอะไรเพิ่ม
npm run test:ui     # 102 — ต้องมี playwright
```
...
| `backend/test/unified-trip-write.test.mjs` | 168 | API ฝั่งเขียนทั้งหมด |
| `frontend/trip-unified-prototype/live-mode.test.mjs` | 102 | หน้าจอจริงบน Chromium |
...
- ⚠️ **ไม่เดาหมวดให้เด็ดขาด** — บิลในทริปเก็บหมวดเป็นข้อความ (อาหาร/ที่พัก) แต่บัญชีจริงใช้ `category_id` · จับคู่ไม่ได้ = **ปฏิเสธพร้อมบอกว่าหมวดไหน** (`unmapped` ในคำตอบ) ดีกว่ายัดลงหมวด "อื่น ๆ" แล้วงบรวมเพี้ยนโดยไม่มีใครรู้ · ส่ง `category_map` และ/หรือ `default_category_id` มาเพื่อจับคู่
...
**ยังไม่ทำ**: หน้าจอยังไม่มีที่ให้เลือกบัญชีปลายทางกับจับคู่หมวด — ตอนนี้เรียกได้จาก API เท่านั้น — `node frontend/trip-unified-prototype/live-mode.test.mjs` · เสิร์ฟไฟล์เอง + API ปลอมที่คืนรูปเดียวกับของจริง **ไม่ยิงฐานจริงเลย** · ครอบคลุมทั้งโหมดปกติ · โหมดจริง · กรณี API ล้ม · และ console สะอาด
